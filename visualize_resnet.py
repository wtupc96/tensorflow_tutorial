...
def train(sess):
    ...
    saver.save(sess, 'checkpoint/resnet/resnet')

def test(sess, do_restore=False):
    ...
    for _ in range(iter_num):
        ...
        # output: (100, 10)
        # result_list: length: 100
        result_list.append(sess.run(output, feed_dict={X: x}))
        total += BATCH_SIZE
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        # train(sess)
        # result_list: (100, 100, 10) ==> (10000, 10)
        result_list = test(sess, do_restore=True)
        cls_list = [[] for _ in range(10)]
        # 10000,10
        for rl in result_list:
            # (100, 10) --> (10000, 10)
            for _r in rl:
                idx = np.argmax(_r)
                cls_list[idx].append(_r)
        ...
        for i in range(len(cls_list)):
            row = i // 5
            col = i % 5
            # cls_list[i]: (sample_num, 10) ==>(10,1)
            # print(np.mean(cls_list[i], axis=0))
            ax[row][col].bar(np.arange(10), np.mean(cls_list[i], axis=0))
