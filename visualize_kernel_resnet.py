...
def resnet_building_block(x, kernel: list, strides: list, out_channel: int, padding: str, name):
    ...
    with tf.variable_scope(name):
        output, w1 = conv_layer(x, kernel, strides, out_channel, padding, name='conv1', do_act=False)
        output = batch_norm(output, 'bn1')
        output = tf.nn.relu(output)

        output, w2 = conv_layer(output, kernel, [1, 1, 1, 1], out_channel, padding, name='conv2', do_act=False)
        output = batch_norm(output, 'bn2')

        if strides[2] != 1:
            _output, _w = conv_layer(x, [1, 1], strides, out_channel, padding, name='conv_', do_act=False)
...
def build_network(x):
    ...
    output, w1 = conv_layer(x, [3, 3], [1, 1, 1, 1], 16, padding='SAME', name='conv1', do_act=False)
    output = batch_norm(output, name='bn1')

    output = tf.nn.relu(output)

    output, w_res1 = resnet_building_block(output, [3, 3], [1, 1, 1, 1], 16, padding='SAME', name='res_block1')
    output, w_res2 = resnet_building_block(output, [3, 3], [1, 1, 1, 1], 16, padding='SAME', name='res_block2')
    # 28*28*16

    output, w_res3 = resnet_building_block(output, [3, 3], [1, 2, 2, 1], 32, padding='SAME', name='res_block3')
    # 14*14*32
    output, w_res4 = resnet_building_block(output, [3, 3], [1, 1, 1, 1], 32, padding='SAME', name='res_block4')
    # 14*14*32

    output, w_res5 = resnet_building_block(output, [3, 3], [1, 2, 2, 1], 64, padding='SAME', name='res_block5')
    # 7*7*64
    output, w_res6 = resnet_building_block(output, [3, 3], [1, 1, 1, 1], 64, padding='SAME', name='res_block6')
    # 7 7 64

    conv_kernels['conv1'] = w1
    conv_kernels['res1'] = w_res1
    conv_kernels['res2'] = w_res2
    conv_kernels['res3'] = w_res3
    conv_kernels['res4'] = w_res4
    conv_kernels['res5'] = w_res5
    conv_kernels['res6'] = w_res6
    
    output = tf.reshape(output, [BATCH_SIZE, -1])
    output = fully_connect(output, 10, name='fc2', do_act=False)
    return output, conv_kernels
...
output, conv_kernels = build_network(X)
